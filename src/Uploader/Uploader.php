...
use LiveCMS\Support\Uploader\Contracts\ModelUploaderInterface as Model;
...
class Uploader
{
    ...
    public function file($field = null)
    {
        ...
        $folder = base_path($this->baseFolder).DIRECTORY_SEPARATOR.$this->folder;
    ...
    protected function processUpload($file)
    {
        ...
        if (file_exists($oldFile = $folder.DIRECTORY_SEPARATOR.$filename.'.'.$extension)) {
            ...
            if ($this->override) {
                ...
                } while (file_exists($folder.DIRECTORY_SEPARATOR.$filename.'.'.$extension));
            }
        };
